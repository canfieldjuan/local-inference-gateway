...
- `deploy/systemd/install-ollama-user.sh` installs only non-secret user configuration after all
  ...
  the root filesystem and incompatible user-manager XDG configuration fail closed, the installed
  unit bytes come from the verified commit blob, and no service lifecycle command exists.
